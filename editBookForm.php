...
/*Due to there being an issue of books where the ISBN value contains an X. Seen in books (Beginning Visual Basic 6 Objects) and (HTML & XHTML: the Definitive Guide).
I have decided that there are two solutions to this problem. One would be trimming the X off the end as seen below, however a more future-proof method would be using
a preg_replace function where all the non-numeric characters are removed incase future books added contain different letters or any array of differing characters*/

//$newBookISBN = trim($bookISBN,"X");		This is a makeshift solution to the problem currently at hand
$newBookISBN = preg_replace('/[^0-9]/', '', $bookISBN); //this is a solution that can be used no matter the new bookISBN's that input into the database
